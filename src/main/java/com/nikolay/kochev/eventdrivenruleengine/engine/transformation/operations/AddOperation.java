...
@Component
public class AddOperation implements TransformationOperation {
    ...
    public void execute(TransformationLoader.Transformation transformation, JsonNode payload, JsonNode result) {
        ...
        String valueToAdd = transformation.value();

        validateInputParameters(path, valueToAdd, result);

        String cleanPath = path.startsWith(ROOT_PREFIX) ? path.substring(2) : path;
        JsonNode currentNode = getValueByPath(result, cleanPath);

        validateFieldExists(currentNode, path);
        validateFieldIsNumber(currentNode, path);

        double addValue = parseValueToAdd(valueToAdd);
        double currentValue = currentNode.asDouble();
        double resultValue = currentValue + addValue;

        setValueAtPath(result, cleanPath, resultValue);
    }

    private void validateInputParameters(String path, String valueToAdd, JsonNode result) {
        if (path == null || path.isEmpty()) {
            throw new TransformationException("ADD operation failed: Path cannot be null or empty");
        }

        if (valueToAdd == null || valueToAdd.isEmpty()) {
            throw new TransformationException("ADD operation failed: Value to add cannot be null or empty");
        }

        if (result == null) {
            throw new TransformationException("ADD operation failed: Result node cannot be null");
        }

        if (!(result instanceof ObjectNode)) {
            throw new TransformationException("ADD operation failed: Result node must be an ObjectNode, but was " + result.getClass().getSimpleName());
        }
    }

    private void validateFieldExists(JsonNode currentNode, String path) {
        if (currentNode == null) {
            throw new TransformationException("ADD operation failed: Field not found at path '" + path + "'");
        }

        if (currentNode.isMissingNode()) {
            throw new TransformationException("ADD operation failed: Field at path '" + path + "' is missing");
        }

        if (currentNode.isNull()) {
            throw new TransformationException("ADD operation failed: Field at path '" + path + "' is null");
        }
    }

    private void validateFieldIsNumber(JsonNode currentNode, String path) {
        if (!currentNode.isNumber()) {
            throw new TransformationException("ADD operation failed: Field at path '" + path + "' is not a number. Current type: " + currentNode.getNodeType() + ", value: " + currentNode);
        }
    }

    private double parseValueToAdd(String valueToAdd) {
        try {
            return Double.parseDouble(valueToAdd);
        } catch (NumberFormatException e) {
            throw new TransformationException("ADD operation failed: Cannot parse value to add '" + valueToAdd + "' as a number", e);
        }
    }
